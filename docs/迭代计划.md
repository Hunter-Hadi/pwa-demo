...
*   **习惯管理**:
    ...
    *   [x] 删除习惯。
*   **待办事项**:
    *   [x] 快速添加 Todo (Next Iteration)。
    *   [x] 勾选完成/未完成 (Next Iteration)。
*   **数据统计**:
    *   [x] 首页展示今日完成率 (例如: "3/5 Completed") (Next Iteration)。
...
## 迭代 2：PWA Core (离线能力) 详细计划

**目标**: 让 Web App 具备原生应用的特性（可安装、离线访问），即使在断网情况下也能打开应用并查看数据。

### 1. 功能清单 (Features)
*   **PWA 基础**:
    *   [x] 配置 `manifest.json` (名称、图标、主题色)。
    *   [x] 配置 Service Worker (使用 `vite-plugin-pwa`)。
    *   [x] 离线缓存静态资源 (HTML, JS, CSS)。
*   **用户体验**:
    *   [x] 自定义安装提示 (Install Prompt) (Optional)。
    *   [x] 离线状态提示 (当网络断开时显示 Toast)。

### 2. 技术实现 (Implementation)
*   **插件**: 使用 `vite-plugin-pwa` 简化 Service Worker 的生成。
*   **策略**:
    *   **App Shell**: 使用 `GenerateSW` 模式，自动预缓存构建产物。
    *   **Runtime Caching**: 对 Google Fonts 或外部 API 使用 `StaleWhileRevalidate`。

### 3. 开发步骤
1.  安装 `vite-plugin-pwa`。
2.  在 `vite.config.ts` 中配置插件。
3.  编写/生成 PWA 图标 (192x192, 512x512)。
4.  在 `index.html` 中添加 meta 标签。
5.  构建并测试离线访问。
